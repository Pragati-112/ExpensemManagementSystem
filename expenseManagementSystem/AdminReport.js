...
const AdminReport = () => {
  ...
  // Fetch all users
  useEffect(() => {
    axios.get("http://localhost:8000/api/admin/report/getreports").then((res) => {
      setUsers(res.data);
    });
  }, []);
  ...
  const generateReport = () => {
    ...
    axios
      .post(`http://localhost:8000/api/user/${userId}/report/add`, reportData)
      .then((res) => {
        setReports(res.data);
      });
  };
  ...
  const deleteReport = (reportId) => {
    axios
      .delete(`http://localhost:8000/api/user/${userId}/report/delete/${reportId}`)
      .then((res) => {
        setReports(res.data);
      });
  };
